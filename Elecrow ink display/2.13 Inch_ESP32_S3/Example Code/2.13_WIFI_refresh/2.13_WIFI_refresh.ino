...
#define txt_size 1274
#define pre_size 1764
...
void UI_price()
{
    ...
    EPD_ShowPicture(0, 0, 248, 24, gImage_conference_5, BLACK); // Display gImage_scenario_home picture on the screen with black background
    ...
    if (SPIFFS.exists("/txt.bin")) { // Check if file /txt.bin exists
        ...
        EPD_ShowPicture(0, 24, 104, 98, txt_formerly, BLACK);
    }
    ...
    if (SPIFFS.exists("/pre.bin")) { // Check if file /pre.bin exists
        ...
        EPD_ShowPicture(104, 24, 144, 98, price_formerly, BLACK);
    }
